...
class CubeObj(PhysObj, pygame.sprite.Sprite):
    def __init__(self, x, y, weight, elasticity) -> None:
        ...
        from Utils.GameScale import CUBE_SIZE
        cube_path = os.path.join(assets_dir, 'CompanionCube_Asset.png')
        self.image = pygame.image.load(cube_path).convert_alpha()
        self.size = CUBE_SIZE
    ...
    def portalWarp(self, portals):
        """Cube teleports through portals - portals only link within the same team"""
        touchedPortal = False
        for portal in portals:
            if portal and self.rect.colliderect(portal.rect):
                ...
                if other_portal:
                    self.rect.center = other_portal.rect.center
                    self.angle = (270 - other_portal.angle) % 360 * math.pi / 180
